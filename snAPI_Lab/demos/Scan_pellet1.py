...
if(__name__ == "__main__"):
    # === Define scan grid ===
    x_range = np.linspace(-0.25, 0, 50)  #np.linspace(min x, max x, N) # N is how many evenly spaced points you want in interval [min x, max x]
    y_range = np.linspace(-0.65, -0.55, 50)   #same for y  #create a NxN grid
    num_x = len(x_range)
    num_y = len(y_range)
    ...
    with nidaqmx.Task() as ao_task:
        ...
        for i, y in enumerate(y_range):
            ...
            for j, x in enumerate(x_seq):
                ...
                sn.timeTrace.measure(500, waitFinished=False, savePTU=False)  #in milisecs #num can be changed
                while not sn.timeTrace.isFinished():
                 time.sleep(0.5)

                counts, times = sn.timeTrace.getData()
                cntRs = sn.getCountRates()
                chan1rate = np.sum(cntRs[1])
                #chan1rate = cntRs[1]
    ...
    np.savetxt("intensity.csv", intensity_map, delimiter=",")
